		...
		private sealed class ActionRenderingExtension : IRenderingExtension, ITargetDevice
		{
			...
			private void ProcessLayoutArea(ILayoutArea area)
			{
				...
				foreach (ILayoutArea child in area.Children)
				{
					...
					{
						var canvas = GraphicsCanvasFactory.Create(GetGraphics(), FontsFactory.Instance, GraphicsCanvasFactory.DefaultSettings);
						var textMetricsProvider = new TextMetricsProvider(FontsFactory.Instance, TextMetricsProvider.DefaultSettings);
                        _graphicsRenderer.Render(new GraphicsRenderContext(this, canvas, textMetricsProvider, RenderersFactory.Instance, null), child);
			...
			{
				var textMetricsProvider = new TextMetricsProvider(FontsFactory.Instance, TextMetricsProvider.DefaultSettings);
				LayoutInfo layoutInfo = new LayoutInfo(report, targetDevice, textMetricsProvider, callback);
