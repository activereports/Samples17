...
#Region "IRenderingExtension implementation"
	...
	Sub Render(report As IReport, streamProvider As StreamProvider, settings As NameValueCollection) Implements IRenderingExtension.Render
		Render(report, streamProvider, settings, CancellationToken.None)
	End Sub

	Public Sub Render(report As IReport, streamProvider As StreamProvider, settings As NameValueCollection, cancel As CancellationToken) Implements IRenderingExtension.Render
		...
		Try
			...
			Using outputStream = primaryStream.OpenStream()
				Using generator As New PdfGenerator(outputStream, pdfSettings.EmbedFonts)
					...
					End If
					renderingCore.Render(pdfSettings.Target, True, New ImageRenderers.PageControl.PageControllerSettings() With {
											.EndPage = endPage,
											.StartPage = startPage
											})
				End Using
