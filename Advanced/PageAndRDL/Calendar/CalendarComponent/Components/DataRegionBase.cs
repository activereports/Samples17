...
using GrapeCity.ActiveReports.Layout;
using GrapeCity.ActiveReports.PageReportModel;
using GrapeCity.ActiveReports.Rendering.RenderingTree.Tools;
using CustomData = GrapeCity.ActiveReports.Extensibility.Rendering.CustomData;
...
namespace GrapeCity.ActiveReports.Calendar.Components
{
	...
	public abstract class DataRegionBase : ReportItemBase, IDataRegion
	{
		...
		/// </summary>
		public bool PageBreakAtEnd => GetValue("PageBreakAtEnd", false);

		/// <summary>
		/// Specifies where the layout should continue.
		/// </summary>
		public EndSection EndSection
		{
			get
			{
				var newPage = (string)PropertyBag.GetValue("NewPage");
				return Enum.TryParse(newPage, true, out StartNewPage result)
					? result.ToEndSection()
					: EndSection.PageBreak;
			}
		}
